...
@show Array(OMEinsum.fast_contract(optcode_loaded.eins,([CUDA.fill(0.5^(0.4), (fill(2, length(ix))...,)) for ix in getixsv(optcode_loaded)]...,)))
Profile.@profile Array(OMEinsum.fast_contract(optcode_loaded.eins,([CUDA.fill(0.5^(0.4), (fill(2, length(ix))...,)) for ix in getixsv(optcode_loaded)]...,)))
Profile.clear()
GC.gc()
Profile.@profile Array(OMEinsum.fast_contract(optcode_loaded.eins,([CUDA.fill(0.5^(0.4), (fill(2, length(ix))...,)) for ix in getixsv(optcode_loaded)]...,)))
